	...
	public function setForceInsert($bool=true){
 	...
 	public function importDictionary($text,$userInsert=false){ 
		...
		$namekeyOrigin='translation.' . $this->origin;
 		...
 		$query='DELETE ' . $transModel->makeT() . '.* FROM ' . $transModel->makeT();
 		$query .=' LEFT JOIN ' . $modelOrigin->makeT() . ' ON ' . $transModel->makeT() . '.imac=' . $modelOrigin->makeT() . '.imac';
 		$query .=' WHERE ' . $transModel->makeT() . '.text=' . $modelOrigin->makeT() . '.text';
 		$query .=' AND ' . $transModel->makeT() . '.auto < 2;';

...
* @license GNU GPLv3 */
 ...
 class Translation_Exportlang_class extends WClasses {
	...
	function getText2Translate($originlgid,$destinationlgid,$wid=0,$onlyNotTranslated=true,$limit=10000){
		...
		$selectedImacs=$this->_getImacs( $wids );
		...
		if( $exist && $onlyNotTranslated){
			
									if( $destinationlgid !=$originlgid){
				...
				if( !empty($selectedImacs)) $transModelOr->whereIn( 'imac', $selectedImacs );
			...
			if(!empty($selectedImacs)) $transModel->whereIn( 'imac', $selectedImacs );
 	...
 	function generateManualContent($content,$wid,$destlgid=1){
		$languageModel=WModel::get( 'library.languages');
	...
	private function _generateFile($contentfile,$format='ini'){
 	...
 	private function _fileHeader(){
		...
		$content .="# --------------------------------------------------------".$this->lineJump;
		$content .="# Published, licensed and distributed by Joobi Limited".$this->lineJump;
		$content .="# Copyright (c) 2008-".date( 'Y') ." Joobi Limited".$this->lineJump;
		$content .="# All rights reserved.".$this->lineJump;
		$content .="# http://www.joobi.co".$this->lineJump;
		$content .="# --------------------------------------------------------".$this->lineJump;

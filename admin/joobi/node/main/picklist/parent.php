...
class Main_Parent_picklist extends WPicklist {
	...
	function create() {
		...
		$allPicklistA = $picklistM->load( 'ol', array( 'namekey', 'did', 'core' ) );
		...
		if ( !empty( $allPicklistA ) ) {
			...
			foreach( $allPicklistA as $onePciklist ) {
				...
				if ( !empty($onePciklist->name) ) $this->addElement( $onePciklist->namekey, $onePciklist->name );

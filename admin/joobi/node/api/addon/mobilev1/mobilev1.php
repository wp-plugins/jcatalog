...
class CMSAPIPage extends APIPage {
	...
	public static function includeRootScript(){
		...
		if(!$includeRootscript){
			$rootscript=JOOBI_FOLDER . '/node/api/addon/' . JOOBI_FRAMEWORK . '/js/rootscript.1.1.js';
			WPage::addScript( $rootscript, 'none' );

			$rootscript=JOOBI_FOLDER . '/node/api/addon/' . JOOBI_FRAMEWORK . '/js/themescript.1.1.js';
			WPage::addScript( $rootscript, 'none' );
	...
	public static function completeURL($url='',$popup=null){
...
abstract class APIApplication {
	...
	public static function cmsUserLang(){
		$lgid=self::_getPremiumLanguage();
		return WLanguage::get( $lgid, 'name' );
	}
	...
	private static function _getPremiumLanguage(){
		static $codelgid=null;

		if( isset($codelgid)) return $codelgid;
		$LanguagesM=WModel::get( 'library.languages' );
		$LanguagesM->remember( 'xLanguagePremiumx', true ); 		$LanguagesM->whereE('premium', 1);
...
class WApplication_mobilev1 {
	...
	function make($entrypoint=null,$params=null,$errorCode=null){
		...
		if( !defined( 'JOOBI_APP_DEVICE_SIZE' )){
				...
				if( !empty($deviceInfoA->isPhone) && $deviceInfoA->isPhone=='true'){
					define( 'JOOBI_APP_DEVICE_SIZE', 11 );
					define( 'JOOBI_APP_DEVICE_TYPE', 'ph' );
				}elseif( !empty($deviceInfoO->isTablet) && $deviceInfoA->isTablet=='true'){
	...
	public static function response($content='',$success=true,$errorMessage=''){	
		...
		if( true===$success){
			...
			$debug=JOOBI_DEBUGCMS || PLIBRARY_NODE_DBGERR || JOOBI_FRAMEWORK_FORMAT_OUPUT || $debugOnly;
				...
				if( empty($keysA)){
					...
					if( !empty($spaceO->menu)){
						...
						if( !empty( $menuDispaly )){
							$menuHTML=$menuDispaly->makeMenu( true );							}					}
					$keysA=WGlobals::get( 'menuKeys', array(), 'global' );
				}
				if( !empty($keysA)){

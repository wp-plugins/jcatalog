...
define( 'JOOBI_TIME_OFFSET', $app->getCfg('offset') * 60 );	
...
if( IS_ADMIN){
	...
	define( 'JOOBI_PRELOAD_JS', true );	}else{
	define( 'JOOBI_INDEX', 'index.php' );
	define( 'JOOBI_INDEX2', 'index.php' );
	define( 'JOOBI_PRELOAD_JS', false );	}
...
abstract class APIPage {
	...
	public static function cmsGetItemId(){
		...
		$wPageID=WGlobals::get( JOOBI_PAGEID_NAME );
...
abstract class APIApplication {
	...
	public static function installThemePath(){
		define( 'JOOBI_URL_THEME_JOOBI', JOOBI_URL_USER . 'theme/admin/joomla30/' );
	}
...
abstract class CMSAPIPage extends APIPage {
	...
	public static function routeURL($link,$absoluteLink='',$indexPassed=false,$SSL=false,$itemId=true,$foption=null,$noSEF=false){
		...
		if( $absoluteLink=='smart'){
			...
			static $includePopJS=false;
			if( !$includePopJS && $index=='popup'){
				$includePopJS=true;
				...
				if( $itemId===true){
					...
					if( $itemId !='none'){
						if( is_numeric($itemId)){
							...
							$item=WPage::cmsGetComponentItemId( $itemId );
			...
			$link=rtrim($link,'&');
				...
				if( substr( $url, 0, 4 ) !='http'){
					$url=JOOBI_SITE . $url;
								...
								$url=( $itemId ) ? WPage::cmsRoute( $link, $SSL ) : $link;
			}			return $url;
		}else{
			$url=rtrim($link,'&');
			return $url;
...
return hPX;
		...
		return ' rev="joobibox" rel="{handler: \'iframe\',size:{x:'.$x.',y:'.$y.'}}"';
	...
	public static function includeMootools(){
		static $includeMootools=false;
			if(!$includeMootools){
				JHtml::_('behavior.mootools');
				$includeMootools=true;
			}	}
	...
	public static function includejQuery(){
		static $includejQuery=false;
			if(!$includejQuery){
					...
					$path=WPage::fileLocation( 'js', 'js/jquery-1.10.2.js', 'api' );
					WPage::addScript( $path );

					$path=WPage::fileLocation( 'js', 'js/jquery_no_conflict.js', 'api' );
					WPage::addScript( $path );
	...
	public static function includeJoobiBox(){
		static $includeJoobibox=false;
		if( !$includeJoobibox){
			WPage::addJSLibrary( 'rootscript' );

			$path=WPage::fileLocation( 'js', 'js/joobibox.js', 'api' );
			WPage::addScript( $path );

			$path=WPage::fileLocation( 'css', 'css/joobibox.css', 'api' );
			WPage::addStyleSheet( $path );


			$includeJoobibox=true;
		}	}
	...
	public static function interpretURL($segments){

		$vars=array();
		$vars['controller']=str_replace(':','-',$segments[0]);
		$i=1;

		while ( !empty($segments[$i])){
	...
	public static function completeURL($url='',$popup=null,$raw=false){
		$url=trim($url);
		$popup=!empty($popup) ? true : WGlobals::get( 'is_popup', true, 'global' );
		if( !empty($url)) $url='?option='. WApplication::name('com', false, $url ) .'&'. $url;
		return $popup ? JOOBI_INDEX2 . ( empty($url) ? '?' : $url . '&' ) . 'tmpl=component'. ( $raw ? '&type=raw' : '') : JOOBI_INDEX . $url;
	}
...
abstract class WApplication extends APIApplication {
	...
	public static function name($short='default',$wPageID=null,$linkController=null){
			...
			$option=WGlobals::getApp();
												$joptionVal=array( 'jcommunity','jiptracker', 'jnotebook', 'jprojects', 'jlinks','jdesign','jfaq','jdiscuss','jservicesprovider','jdefender', 'infusionstats', 'jurlauncher','jcenter',
 ...
 'jsubscription','jtestcases','jcontacts','jcatalog','jdownloads', 'jdloads','jclassifieds','jdatabase','jtodo', 'jchecklist','jdocumentation','jdistribution','jbounceback','jdictionary','jforum','jlicense','jnewsletters','jcampaign', 'joomfusion' );
			...
			$myOption=substr( $option, 4);
				...
				if( !empty($wPageID) && $wPageID !==true){
										$link=WPage::cmsGetLinkBasedItemId( $wPageID );
					if( !empty($link)){
	...
	public static function mainLanguage($return='lgid',$force=false,$suggestedLang=array(),$location='site'){
		...
		if( empty($lang) || $force){
			...
			$langCode=array( APIApplication::cmsMainLang( $location ));				if( !empty($langCode)){
	...
	public static function setWidget($object){
		...
		switch ( strtolower($object->type)){
			...
			default :
				$message=WMessage::get();
				$message->codeE( 'The type specified does not exist in Joomla 1.6. :' . $object->type );
				return false;
				break;
		}
	...
	public static function createMenu($name,$menuParent,$link,$option,$client=1,$access=0,$level=0,$ordering=0,$param=null){
		...
		$APIHelperC=new Api_Joomla16_Helper_addon;
...
class WApplication_joomla16 {	
	...
	public static function make($entrypoint,$params=null){
		...
		if( $processApplication){
			if( JOOBI_PRELOAD_JS && $entryTypeText=='components'){
								WPage::jsPreload();
			}
		...
		$checkingUrl=WPage::linkAdmin(WGlobals::currentURL());

		if( !empty($soundMusic->alex)) $soundMusic->alex=str_replace( 'ip tracker', 'jiptracker', $soundMusic->alex );

		if( WPref::load( 'PLIBRARY_NODE_ENABLESOUND' ) && is_object($soundMusic) &&  ! empty($soundMusic->alex) && ($soundMusic->alex==$checkingUrl || strpos($checkingUrl, $soundMusic->alex)===0 )){
			...
			WGlobals::setSession( 'installRedirectInfo', 'alex' , '' );
		}
				return $content;

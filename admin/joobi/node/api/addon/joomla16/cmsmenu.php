...
* @license GNU GPLv3 */
 ...
 class Api_Joomla16_Cmsmenu_addon {
		...
		if( is_array($exts) && count($exts)>0){
			...
			foreach( $exts as $ext){
				...
				if( method_exists( $addon, $method )){
					...
					$sql=WModel::get('apps.dependency');
	...
	public function loadExtension($id,$option=null){
		...
		if( 'com_modules'==$option || 'com_advancedmodules'==$option){
			$modulesT=WTable::get( '#__modules', '', 'id' );
	...
	public function editExtensionPreferences(){
		...
		$extensionO=$this->loadExtension( $id );
		...
		if( empty($widgertId)){
			...
			if( !empty($mainWidgetC)) $mainWidgetM->wgtypeid=$mainWidgetC->getWidgetTypeID( str_replace( '.module', '', $extensionO->namekey ));

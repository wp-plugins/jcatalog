...
class Api_Joomla16_User_addon {
		...
		if( IS_ADMIN){
			...
			if( !empty($comeBackHere)){
				...
				if( $comeBackHereCount > 1){	
					...
					if( $j16Fix || ( $option=='com_users' && $view=='profile' )){
						WGlobals::setSession( 'Joobi_Users_ComeBack', 'JoomlaUsers', '' );
					...
					}elseif( $option=='com_users'){							$comeBackHereCount++;
				...
				}else{ 					$option=WGlobals::getApp();
					if( $option=='com_users'){							$comeBackHereCount++;
						WGlobals::setSession( 'Joobi_Users_ComeBackCount', 'JoomlaUsers', $comeBackHereCount );
					}else{
											}
				}
	...
	public function clearSession($space='site'){
		...
		$componentM=WTable::get( '#__session', '', 'session_id' );

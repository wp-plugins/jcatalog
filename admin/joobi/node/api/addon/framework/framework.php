...
class APIPage {
	...
	public static function getSpoof($alt=null){
		return md5( WUser::get( 'uid' ) . WUser::getSessionId());
	}
...
return hPX;
	...
	public static function includeRootScript(){
		...
		if( !$includeRootscript){
			self::includejQuery();
			$rootscript=JOOBI_SITE_PATH . JOOBI_FOLDER . '/node/api/addon/' . JOOBI_FRAMEWORK . '/js/rootscript.1.1.js';
	...
	public static function completeURL($url='',$popup=null){
		...
		return $popup ? JOOBI_INDEX2 . ( empty($url) ? '?' : $url . '&' ) . 'tmpl=component' : JOOBI_INDEX . $url;
	}
...
class WApplication_framework {
	...
	function make($entrypoint=null,$params=null){
		...
		if( !empty($ecoContent)) WMessage::log( $ecoContent, 'error-buffet-ouput' );
		ob_end_clean();

		$debug=JOOBI_DEBUGCMS || PLIBRARY_NODE_DBGERR;

...
class Api_Wp4_User_addon {
	...
	public function ghostAccount($email,$password,$name='',$username='',$automaticLogin=false,$createJoobiUser=true,$sendPwd=false,$extraPrams=null){
		...
		$pageId=( !empty( $extraPrams->pageId ) ? $extraPrams->pageId : APIPage::cmsGetItemId());
	...
	public function syncUser($user,$isnew,$success,$msg=''){
		...
		if( !empty( self::$_onlySyncOnceA[$user['id']] )) return true;
	...
	public function clearSession($space='site'){
		...
		$componentM=WTable::get( '#__usermeta', '', 'umeta_id' );
	...
	public function deleteSession($sessionID){
		...
		$componentM=WTable::get( '#__usermeta', '', 'umeta_id' );
		if( is_array($sessionID)) $componentM->whereIn( 'session_tokens', $sessionID );
		else $componentM->whereE( 'session_tokens', $sessionID );
		$componentM->delete();

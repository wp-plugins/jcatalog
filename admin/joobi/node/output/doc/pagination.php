...
class WPagination {
	...
	public function displayNumber($formName,$task='',$increament=null,$maxDisplayNB=0){
		...
		if( !empty($increament) && $increament > 1){				$nextTimeStop=false;
			$maxLoop=PLIBRARY_NODE_MAXLIMIT + 2;
			for( $i=1; $i < $maxLoop; $i++){
		...
		if( WGet::isDebug()){
			...
			$namekey='WZY_' . WGlobals::count('f');
				...
				$joobiRun='return ' . WPage::actionJavaScript( $task, $formName, array(  'limitstart'=> 'limitstart' . $this->yid ), false, $namekey );			$dropdownPL=WView::picklist( null, $joobiRun, $didlist );
	...
	private function _oneLink($text,$value,$current=false,$class='',$class2='',$off='',$index=null){
		...
		$pageO->text=$text;
				...
				$joobiRun='return ' . WPage::actionJavaScript( $this->_defaultTask, $this->formName, array('pagi'=>true,'limitstart'=>'limitstart'. $this->yid ), $value, $jsNamkeyA[$key] );

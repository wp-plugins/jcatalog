...
class Output_Doc_htmllistings extends Output_Doc_Document {
	...
	private function _renderContentTable(){
		...
		if( isset( $this->htmlObj->formObj )){
			...
			if( !$this->htmlObj->manualDataB){
				...
				$this->htmlObj->formObj->hidden( 'trucs[s][pkey]', $pKey );

			 	$this->htmlObj->formObj->hidden( 'trucs[s][mid]', $this->htmlObj->sid );
			...
			if( !empty($this->allDataRowsA)){
				...
				foreach( $this->allDataRowsA as $rowKey=> $row){
					...
					if(  isset($this->htmlObj->_model) && $this->htmlObj->_model->getType()==40 ){
						...
						if(  !empty($this->htmlObj->myParentA) && $row->$parent['parent']=='0'){

							continue;

						}elseif( isset($row->parent) && $row->parent=='0'){
					...
					if( defined('JOOBI_TR_C_ROW1')){ 
						if( $this->rowNumber & 1){ 

							$t->tr_c=JOOBI_TR_C_ROW2;

						}else{


							$t->tr_c=JOOBI_TR_C_ROW1;
						...
						foreach( $elemtVal as $colNum=> $listing){
							...
							&& !(isset($this->htmlObj->_params->_extras['unlinked'])
							 ...
							 && in_array($listing->map, $this->htmlObj->_params->_extras['unlinked']))){
								...
								if( empty($listing->dontConvertLien)) $link=$outputLinkC->convertLink( $listing->lien, $row, $linkFilters, $this->htmlObj->_model, $this->htmlObj->mapListA );
							...
							if( $listing->parent==0){
								...
								if( isset( $listing->anchor )){
									...
									if( isset($row->$complextAnchorMap)) $htmlData='<a class="anchor" name="'. $row->$complextAnchorMap.'">' . $htmlData;
	...
	private function _ordering($text,$orderMap,$ordering=false,$description=''){
		...
		if( $ordering===false){
			...
			$h .='href="#" onclick="'.$joobiRun.'" title="' . $titleHeader .'">';

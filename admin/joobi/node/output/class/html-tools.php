...
class WSpan extends WElement {
	...
	function create(){
		...
		$this->content='<span' ;
		if(isset($this->classes)) $this->content .=' class="'.$this->classes.'"';
		if(isset($this->title)) $this->content .=' title="'.$this->title.'"';
		if($this->style) $this->content .=' style="'.$this->style.'"';
		if($this->align) $this->content .=' align="'.$this->align.'"';
...
hiddenJSTreeInput.val(jQuery(this).attr("name"));
	...
	function create($list,$tag_name,$tag_attribs=null,$propKey='value',$propText='text',$selected=null,$idtag=null,$translate=false,$arrayType=false,$onChange=false){
		static $addJS=true;
		static $count=1;
		...
		if( $useFancy && $addJS){
			...
			$addJS=false;
				...
				if( !isset($idtag)) $idtag='wz_id' . $count;
		$count++;


				$this->content='<select name="' . $tag_name . '"';
	...
	private function _options($list,$propKey='value',$propText='text',$selected=null,$translate=false,$arrayType=false){	
		...
		return ( $optionGroup ) ? '</optgroup>' . $html : $html;
...
class WRadio {	
	...
	function create($list,$tag_name,$tag_attribs=null,$propKey='value',$propText='text',$selected=null,$idtag=null,$disable=false,$arrayType=false,$radioType=false,$colnb=0,$reqChecked=false){	
		...
		$data->tagID=str_replace( array( '.','][', ']', '['), '_', $idtag );
...
class WList {
	...
	function create($list,$baseLink,$selected=null,$tag_attribs=null,$propKey='value',$propText='text'){	
		if( empty($list)) return '';
...
class WForm extends WElement {
	...
	var $option=null; 		var $act=null;			var $task=null;		
	...
	public static function getPrev($map,$sid=0){
		...
		if( !empty($sid)){
			...
			$previous=( isset($trucs[$sid])) ? $trucs[$sid] : array();
			if( !empty($previous)){
				...
				if( substr( $map, 1, 1 )=='['){
					...
					if( isset($previous[$subMap][$map])) return $previous[$subMap][$map];
						...
						$editorMap='zdtr_trucs_' . $sid . '__' . $map . '_';
		...
		if( substr( $map, 1, 1 )=='['){
			...
			if( empty($map)) return;
						...
						$trucs=WGlobals::get( 'trucs', array(), '', 'array' );
			if( isset($trucs[$subMap][$map])) return $trucs[$subMap][$map];
	...
	public function hidden($name,$value='',$overwrite=false,$addToSecurityCheck=false){
			...
			return;
				...
				if( $addToSecurityCheck && substr( $name, 0, 6 )=='trucs['){
			$tempString=rtrim( substr( $name, 6 ), ']');
		...
		if( is_array($value)){
			...
			foreach( $value as $oneValue){
				$newName=$name . '['.$i.']';
				if((!empty($newName) && !isset($this->_hidden[$newName])) || (!empty($newName) && $overwrite)) $this->_hidden[$newName]=$oneValue;
				$i++;
			}		}else{
	...
	public function eForm(){
		...
		if( $isPopUp ) $this->hidden( 'isPopUp', 'true' );
			...
			foreach( $keys as $key){
																...
																$idHidden=$this->getIdHidden( $key );
				$h .='<input type="hidden" id="'.$idHidden.'" name="'.$key.'" value="' . $this->_hidden[$key] . '"/>' . $this->crlf;
			}		}
	...
	public function getIdHidden($map){
		if( strstr($map,'trucs[')){
			$pattern='/[a-zA-Z0-9]+(?=\])/';
...
class WLink extends WElement {
	...
	function make($url='',$notUsed=null){
		...
		if($this->title) $attribs .=' title="'.$this->title.'"';

...
class Output_Process_class extends WClasses {
 	...
 	private $_pattern="#{ *widget[^{}]*}#i";	 	private $_supportOldTags=true;
	...
	private $_member=null; 	
 	...
 	private $_wdigetParamsReturnedA=array();
	...
	private function _replaceTagsU(&$object,$listTagsA,$userTag=false){
		...
		foreach( $listTagsA as $key=> $oneTagA){
			...
			if( is_object($tagC) && method_exists( $tagC, 'process' )){
				...
				foreach( $oneTagA as $cleaKey=> $cleanValue){
					...
					$cleanOneTagA[urldecode($cleaKey)]=$cleanValue;
				}				$resultFomProcessA=$tagC->process( $cleanOneTagA );
				$currentVAl=current($cleanOneTagA);
				if( !empty( $currentVAl->widgetid )) $this->_wdigetParamsReturnedA[$currentVAl->widgetid]=$currentVAl;
		...
		if( !empty($tagsA)){
			foreach($tagsA as $tag=> $replace){
				...
				$this->_replace( $tagsReplaced, $replaced, $object );
	...
	public function returnParams($wdigetID=0){
		if( empty($wdigetID)) return $this->_wdigetParamsReturnedA;
		elseif( !empty( $this->_wdigetParamsReturnedA[$wdigetID] )){
			return $this->_wdigetParamsReturnedA[$wdigetID];
		}else{
			return null;
		}	}
	...
	private function _loadExtensions($wantedTag){
		...
		if( empty($this->_tagExtensions)){
			...
			if( $caching > 0){
				$cache=WCache::get();
				$this->_tagExtensions=$cache->get( 'loadedx3z7xTags', 'Widget' );
			}
		}
		if( isset($this->_tagExtensions[$wantedTag] )) return true;
			...
			if( $caching > 0){
				...
				$cache->set( 'loadedx3z7xTags', $this->_tagExtensions, 'Widget' );
			}
		}
		...
		if( isset($this->_tagExtensions[$wantedTag])) return true;
				...
				$message=WMessage::get();
		$message->codeE( 'No tag is linked to the type ' . $wantedTag, array(), 0 );			return false;
	...
	private function _replace($tagsReplaced,$replaced,&$object){
		...
		if( is_string($object)){
			...
			return $object; 		}
		if( is_array($object) || is_object($object)){
			foreach( $object as $key=> $suite ) $object[$key]=$this->_replace( $tagsReplaced, $replaced, $suite );
		}
	}

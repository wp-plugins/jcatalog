...
class Output_Mlinks_class extends WView {
	...
	protected function getElements($yid,$params=null){
		...
		$extraParams->select=array( 'type', 'action', 'position', 'parent', 'params', 'private', 'yid', 'rolid', 'ordering', 'namekey', 'icon', 'faicon', 'color', 'xsvisible', 'xshidden', 'devicevisible', 'devicehidden' );
	...
	public static function loadButtonFile(&$button){
		...
		( !empty($button->action) && in_array( $button->action, array('copy', 'delete', 'deleteall', 'copyall' )) )){
		...
		if( ! method_exists( 'WView', 'includeElement')) return $obj;
...
class WButtons_standard extends WElement {
	...
	protected function setButtonJS(){
		...
		}elseif( !empty($this->buttonO->popclose)){
			 $paramsArray['popclose']=true;
		}

...
class Output_Filters_class extends WClasses {
	...
	public function addFilterToView(&$layout,&$sql,$conditionsToRemoveA=array()){
		...
		$this->_myFilters=$this->_cacheFitlers( $this->_yid, 'view' );
	...
	public function addFilterToList($lsid,&$sql){
		...
		if( !isset( $filtersA[$lsid] )){
			...
			$this->_myFilters=$this->_cacheFitlers( $lsid, 'list' );
	...
	public function jviewFilters($type){



		$filterM=WModel::get( 'library.viewfilters', 'object' );

		$filterM->select( array( 'sid',  'type', 'map', 'condopr','logicopr', 'ref_sid', 'refmap', 'typea', 'typeb', 'bktbefore', 'bktafter','namekey','params', 'requiresvalue', 'requirednode' ));
		...
		$filterM->where( 'level', '<=', WGlobals::getCandy());
	...
	private function _cacheFitlers($key,$type){
		...
		$cachingType=( $caching > 5 ) ? 'cache' : 'static';
		...
		$cachedFilter=$cache->get( $usedKey, 'Filter', $cachingType );

		if( !empty($cachedFilter)) $getModel=false;



		if( $getModel){

			$data=$this->jviewFilters( $type );



			$cache->set( $usedKey, $data, 'Filter', $cachingType );
		...
		foreach( $this->_myFilters as $filter){
			...
			case 7:		
				...
				|| $filter->typeb!=90){
					...
					$this->_sql->checkAccess( $this->_sql->getAs( $filter->sid ), $filter->bktbefore, $filter->bktafter, $filter->logicopr );

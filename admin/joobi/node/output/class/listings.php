...
class Output_Listings_class extends WView {
	...
	function create(){
		...
		WText::t('1216230542PQQV');
			...
			if( !empty($trucs['x']['jdoctype'])) $documentTypeInURL=$trucs['x']['jdoctype'];
		...
		$this->_context=$this->yid;
				$currentOrder=WGlobals::getUserState( "wiev-$this->yid-fltr_rdr", 'sorting', '', 'string' );

		$currentOrderArr=explode( '|', $currentOrder );
		...
		$pageNumber=WGlobals::get( 'pagenb'.$this->yid, 0 );

		if( !empty($pageNumber)){
			$maxItems=WGlobals::get( 'total'.$this->yid, 0 );
			...
			if( !empty($this->_model)){
				...
				if( isset( $this->queryInfoWhere)){
					...
					if( $this->nestedView===true){							$addToForm=array( 'radioyn' );
						...
						for($index=0; $index < $sizeChamps; ++$index){
							$valFilterDrop=@$valueChamp[$this->champsA[$index]->map];
					...
					$AdvSearchStatus=WGlobals::getUserState( 'wiev-' . $this->yid . '-adv_srch' , 'viewIDadv', '', 'string' );
							...
							$this->_mywordssearched=$outputHelperC->convertSearchTerms( $this->searchWord );
									...
									$columnInstance->searchQuery( $this->_model, $oneSeach, $this->searchWord );
								}
					...
					$mess=WMessage::get();
					$mess->codeW( 'No listing element found for the view: ' . $this->yid );
	...
	protected function getElements($yid,$params=null){
		...
		return parent::getSQLparent( $yid, 'viewlistings', 'lid', $extraParams, true, true );

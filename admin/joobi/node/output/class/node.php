...
class WView extends WElement {
	...
	public static $directionForm=null;	
	...
	function __construct($layout){	
		...
		if( empty($layout->onlyLoadParent)){
			...
			$this->addProperties( $originalLayout );
						...
						if( !empty( $this->fdirection )){					self::$directionForm[$this->yid]=$this->fdirection;
			}elseif( !empty( $this->formDirection )){
				self::$directionForm[$this->yid]=$this->formDirection;
			}else{
				self::$directionForm[$this->yid]=null;
			}
			...
			if( true !==$themeExist ) return $themeExist;
				...
				if( !empty($spaceO->menu) && !empty($this->horizmenu)){
					...
					if( 'edit' !=$task){
												$nodeNameKeyId=WExtension::get( $this->wid, 'namekey' );
			...
			self::$viewID[]=$this->yid;
					...
					if( isset( $this->fname )){
						...
						$this->formObj->name=$formId;
										...
										if( ! IS_ADMIN){
							$spoof_alt=1;
						$this->formObj->hidden( 'wz_sp_type' , $spoof_alt );
						$this->formObj->hidden( WPage::getSpoof($spoof_alt) , '1' );
					}
	...
	public static function includeElement($element,$location=null,$custom=false,$force=false,$base=null){
		...
		if( !class_exists( $classname ) || $force){					WLoadFile( $element, $location );
	...
	public static function getURI($keepEid=true){
		$uri=array();
			...
			foreach( $keysA as $key9=> $prop){
				if( $key9=='trucs[s][pkey]' ) break;					if( $key9=='trucs' || $key9=='joobJSrequired' ) continue;
			...
			$myOption=WApplication::name('com');
				...
				if( ! IS_ADMIN && JOOBI_FRAMEWORK_TYPE=='joomla' && empty($arguments[JOOBI_PAGEID_NAME])){
					$arguments[JOOBI_PAGEID_NAME]=APIPage::cmsGetItemId();
				}
	...
	public function make($notUsed=null,$notUsed2=null){
		...
		if( !empty($this->_bottom)) $this->content .='<div class="jbotextra">' . implode( "", $this->_bottom ) . '</div>';
			...
			$this->_setPageTitle();
				...
				if( $this->menu > 0 && $this->nestedView===false){
			$topMenu='';
			$bottomMenu='';
						if( $this->menu > 1 && $this->type !=204){					$menuHTML=$this->makeMenu( true );
	...
	public function makeMenu($renderMenu=true){
		...
		if( !empty($this->menu2Remove)) $viewMenuV->removeElements( $this->menu2Remove );
				...
				if( ! $renderMenu && ( $layout->type >=200 && $layout->type < 240 )){				return $viewMenuV->getOnlyElements();
	...
	protected static function getSQLparent($yid,$table,$id,$params=null,$name=true,$description=false,$message=false){

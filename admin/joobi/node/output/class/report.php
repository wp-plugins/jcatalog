...
class Output_Report_class {
	...
	public static function reportQuery($task,&$model,$noQuery=true,$map='',$sid='',$orderDate='',$yid='',$firstFormName='',$reportnosetinterval=false){
		...
		static $endTime=0;
						...
						$interval=(!empty( $trucs['x']['interval'] ) ? $trucs['x']['interval'] : WGlobals::get('interval'));
		if( empty($interval)){
		...
		$currentPresetDate=(!empty( $trucs['x']['presetdate'] ) ? $trucs['x']['presetdate'] : 0 );
		if( empty($currentPresetDate)){
			...
			if( empty($currentPresetDate)) $currentPresetDate=30;			}		WGlobals::setSession( 'graphFilters', 'presetdate', $currentPresetDate );
						...
						if( $task=='generate' && (!empty($trucs['x']['startdate']) || !empty($trucs['x']['enddate'] ))){
				$trucs=WGlobals::get('trucs');
				$start=$trucs['x']['startdate'];
				$end=$trucs['x']['enddate'];
				...
				$endTime=$currDate=time() - JOOBI_TIME_OFFSET * 3600;																					switch( $currentPresetDate){

...
class WPicklist_main extends  WObj {
	...
	public $defaults=null; 	public $params=null; 	var $name=null; 	private $_dropListsA=array(); 	var $_picklistLists=null;
	...
	public function getValueFromIdentifier($identifier,$addIdenfier=true){
		...
		if( $addIdenfier){
			$value=$this->_didLists[0]->namekey . '_' . $identifier ;
		}else{
			$value=$identifier;
		}
	...
	function create(){
		...
		if( !empty($this->_didLists[0])){
			...
			foreach( $this->_didLists as $dropdown){
				...
				if( $dropdown->type=='0' || $dropdown->type=='1'					){						$dropValueIds[]=$dropdown->did;
					$exist=true;
				}			}
			...
			foreach( $this->_didLists as $myDID=> $dropdown){
				...
				if( empty($dropdownMap)){
					if( !empty($dropdown->sid) && !empty($dropdown->map)) $dropdownName='trucs[' . $dropdown->sid . '][' . $dropdown->map . ']';
					else $dropdownName='map' . $dropdown->did;
				}else{
					$dropdownName=$dropdownMap;
				}
								if( !empty($this->name2Use)) $dropdownName=$this->name2Use;
				$dropdownTagId=( isset($this->params->idLabel)) ? $this->params->idLabel : $dropdownName . 'w' . $IDcount . 'm' . $dropdown->did;
				$IDcount++;
	...
	private function _makeHTML($dropdown,$dropdownName,$dropdownTagId,$myDID){
		...
		switch( $dropdown->outype){
			case '0' : 			case '6' : 			case '1' : 				$HTMLDrop=new WSelect();
				...
				$this->_Hdrop=$HTMLDrop->create( $this->_oneDrop, $dropdownName, $this->_htmlType, $this->_idName, $this->_captionName, $this->_initialValue, $dropdownTagId, $this->translate, $this->_arrayType );
				break;
			...
			case '2' : 
				...
				$this->_Hdrop=$HTMLDrop->create($this->_oneDrop, $dropdownName, $this->_htmlType, $this->_idName, $this->_captionName, $this->_initialValue, $dropdownTagId, false, $this->_arrayType, false, $colnb );
				break;
			...
			case '3' : 			case '8' :
				...
				if( !empty($this->_classes)) $HTMLDrop->classes=$this->_classes;
				$this->_Hdrop=$HTMLDrop->create( $this->_oneDrop, $dropdownName, $this->_htmlType, $this->_idName, $this->_captionName, $this->_initialValue, $dropdownTagId, false, $this->_arrayType, true, $colnb, $reqChecked );
				break;
			case '7' : 				$colnb=( !empty($dropdown->colnb)) ? $dropdown->colnb : 3;
				$heightBigBox=$colnb * 26;
				$HTMLDrop=new WRadio();
				if( !empty($this->_classes)) $HTMLDrop->classes=$this->_classes;
				$HTMLDrop->radioStyle='checkBoxMultipleSelect';
				$colnb=1;
				$this->_Hdrop=$HTMLDrop->create( $this->_oneDrop, $dropdownName, $this->_htmlType, $this->_idName, $this->_captionName, $this->_initialValue, $dropdownTagId, false, $this->_arrayType, true, $colnb);
				break;
			...
			case '2' : 				$value=$this->_makeQueryType( $dropdown, true );
				...
				if( !empty($dropdown->external)){
					...
					if( $dropFunction===false ) $returnedObj->status=false;
						...
						if( sizeof(@$dropFunction->picklistValues) > 1){
	...
	private function _makeTableType($dropdown){
		$this->_initialValue=null;
				...
				if( isset( self::$_picklistsValuesA[$dropdown->did] )){
			$this->_idName='id';
			$this->_captionName='name';
			$this->_default=$dropdown->map;
			$this->_oneDrop=self::$_picklistsValuesA[$dropdown->did];
	...
	private function _makeTableTypeOne($dropdown){
		...
		if( !empty($this->defaults) && $hasValuesB){
			...
			$picklistM->rememberQuery();
			$picklistM->makeLJ( 'library.picklistvaluestrans', 'vid');
			$picklistM->whereLanguage(1);
			$picklistM->select( 'name', 1 );
			$picklistM->whereE( 'did', $dropdown->did );

			if( is_array($this->defaults)){
				$picklistM->orderBy( 'ordering' );
				$picklistM->whereIn( 'value', $this->defaults );
				$result=$picklistM->load( 'lra' );
			}else{
				$picklistM->whereE( 'value', $this->defaults );
				$result=$picklistM->load( 'lr' );
			}		}else{
			$result=null;
		}
	...
	private function _createExternalPicklistInstance(&$droplist,$onlyOne=false){
		...
		WLoadFile( $myFuntion.'.picklist.'.$myFile , JOOBI_DS_NODE );
		$className=ucfirst( $myFuntion ) . '_' . ucfirst( $myFile ) . '_picklist';
	...
	private function _getSQLFromDB($dIds,$short=false){	
			...
			if($short){
				$picklistM->select( array( 'did', 'rolid','namekey','sid','ref_sid','type','outype','external','allowothers','first_value','first_caption','params','map','parent','isparent','first_all','lib_ext','wid', 'reload', 'colorstyle' ));
			}
			$picklistM->whereE('publish', 1 );
						$namekey=WGlobals::get('extensionKEY', null, 'global' );
			if( empty($namekey)) $namekey=WExtension::get( WGlobals::get('extensionID', null, 'global' ), 'namekey' );
	...
	private function _getDropValues($values,$published=true){
		...
		foreach( $this->_didLists as $lookOneDID){
			if( $lookOneDID->did==$sampleDID){
				break;
			}		}
...
class WPicklist extends WObj {
	...
	protected function setDefault($values,$overwrite=false){
		...
		if( empty( $this->defaultValue[$this->_params->did] ) || $overwrite===true){
			...
			@$this->defaultValue[$this->_params->did]=$values;
		}
	}

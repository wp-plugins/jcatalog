...
class Output_Theme_class extends WClasses {
	...
	public function createLayout(&$productA,$tagParams=null){
		...
		if( !empty($tagParams->nodeName)) $this->nodeName=$tagParams->nodeName;
		...
		if( !empty($tagParams->layoutname)) $tagParams->layout=$tagParams->layoutname;
			...
			if( !empty($tagParams->widgetSlug)) $this->_tagID=$tagParams->widgetSlug;
			else {
				$this->_tagID='jwdgt';
			}
			if( !empty($tagParams->widgetID)){
				$this->_tagID .=$tagParams->widgetID;
			}else{
				$this->_tagID .=$count;
			}
		}
	...
	private function _createTable(&$productA,$tagParams=null){
		...
		if( $this->hasImage && empty($tagParams->showNoImage)){
			...
			foreach( $productA as $product){
				Output_Theme_class::setImageSize( $product, $tagParams, $this->_tagID );
				$newProducA[]=$product;
			}
		}else{
			$newProducA=$productA;
	...
	public function mouseOverParams(&$tagParams){
		static $loadMeB=true;
				...
				if( !empty( $tagParams->mouseOver )){
						$mouseOVerA=WTools::pref2String( $tagParams->mouseOver );
	...
	public static function callTheme($product,$tagParams,$isTable=false){
		...
		if( empty($tagParams->themeType)) $tagParams->themeType='tag';
		if( !in_array( $tagParams->themeType, array('tag', 'node')) ) $tagParams->themeType='tag';
		...
		if( !empty($tagParams->uniqueSection)){
			...
			if( !empty($tagParams->layoutNbColumn)){
				...
				self::$countID++;
				$tagParams->uniqueID=$tagParams->uniqueSection . '_' . self::$rowNb . '_' . self::$columnNb;

			}else{
				$tagParams->uniqueID=$tagParams->uniqueSection . '_' . 1 . '_' . self::$countID;
			}
		}

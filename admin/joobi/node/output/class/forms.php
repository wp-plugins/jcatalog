...
function create(){
		...
		$complexForm=false;
			...
			if( !empty($sid)){
				...
				$this->_data->$pKey=$this->_eid;
						...
						if( $this->type==1 && WGlobals::getCandy() > 20 && ( !isset($this->_model->_ckout) || ( isset($this->_model->_ckout) &&  !$this->_model->_ckout)) ) $this->_checkoutItem();
			...
			$securityCheckIni=array();
				...
				switch( substr($map, 0, 2)){
					...
					default:							if( !empty($multiform->map )
						&& $multiform->sid!=0
						&& $addmeIn
			...
			if( $this->getModel){
				...
				$valueTable=array();
						...
						switch( $preInital){
							...
							case '{widget:time':									$this->_data->$mapComplex=time();
								break;
							case '{widget:user':									$keyInitalA1=explode( '=', $specInitalA[1] );
				...
				if( isset($this->formObj)) $this->formObj->hidden( 'trucs[s][ftype]', ( $complexForm ? '1' : '0' ));
	...
	$boxHTML=array();
		...
		if( $this->_parent || $this->_area){
			...
			$gotParent=false;
				...
				foreach( $allElements as $area=> $areaElements){
					...
					$pt=$value->parent;
					if( $pt > 0 ) $gotParent=true;
					$list=@$elements[$pt] ? $elements[$pt] : array();
					array_push( $list, $value );
					$elements[$pt]=$list;
				}				$elements=array_reverse( $elements, true );
				...
				$HTML='';
				$message=WMessage::get();
				$message->codeE( 'No forms elements could be displayed contact developer error 2' );
			}
		}
		...
		if( isset($this->formObj)){
			...
			if( !empty($this->sid)) $this->formObj->hidden( 'trucs[s][mid]' , $this->sid );	
						if( isset($this->_model)){
								$mySID=$this->sid;
				...
				if( PLIBRARY_NODE_CKFRM && PLIBRARY_NODE_SECLEV > 1){
					...
					$this->formObj->hidden( 'trucs[s][cloud]' , $finalSecure );
				}
			}
			$keyVal=( isset($keyVal)) ? $keyVal : '';
			if( isset($this->_newEntity) && empty($keyVal)) $this->formObj->hidden( 'trucs[s][new]', (int)$this->_newEntity );
...
private function _renderElements(&$HTML,$oneElements,$elements=null,$reset1=false,$keys=null){
	...
	foreach( $oneElements as $key=> $element){
		if( is_array($element)){
			...
			if( !empty($element->xsvisible)){
				if( JOOBI_APP_DEVICE_SIZE < $element->xsvisible ) continue;
			}			if( !empty($element->xshidden)){
				if( JOOBI_APP_DEVICE_SIZE < $element->xshidden ) continue;
			}		}
		if( JOOBI_APP_DEVICE_TYPE){
			if( empty($element->devicevisible)) $element->devicevisible='';
			if( empty($element->devicehidden)) $element->devicehidden='';
			if( ! WView::checkDevice( $element->devicevisible, $element->devicehidden ))  continue;
		}
		...
		$holaParams->id=WGlobals::filter( $this->controller . $this->yid . $element->fid, 'alnum' );
		...
		$value='';
			...
			if( $wildPrefix=='c[' || $wildPrefix=='u['){
				...
				if( $wildPrefix=='p['){
					...
					if( $wildPrefix=='c['){						
						$value=WPref::load( $configElement );
					...
					if( !empty($element->initial)){
						...
						$valueFromRequest=( !empty($trucs) && isset($trucs['x'][$myRealMap])) ? $trucs['x'][$myRealMap] : null;
				...
				$complexMap='trucs'.'['.$map.']';
			}else{
				$complexMap='trucs'.'['.$element->sid.']'.'['.$map.']';
			}
		}else{
			$complexMap=$value='';
		}
		if( isset($element->accflip)){
			...
			if( ! method_exists( 'WView', 'includeElement')) continue;
			...
			if( class_exists($classname)){
				...
				$message->codeE( 'Check the view :' . $this->yid );
				continue;
			}
			...
			$element->editItem=$this->editItem;
				...
				if(( (int)$element->level >  WGlobals::getCandy())){
					...
					if( !isset($level[$element->level])){
						$plusPRO=WType::get( 'apps.level' );
						$levelName[$element->level]=$plusPRO->getName($element->level);
					}
					$element->levelReadOnly=$levelName[$element->level];
										$highLevel=true;
			...
			$field->idLabel=( isset($idLabel)) ? $idLabel : $complexMap;
			$field->idLabel=str_replace( '.', '_', $field->idLabel );
			$field->idLabel=str_replace( '][', '_', $field->idLabel );
			$field->idLabel .='_' . $this->yid;

									if( 151 !=$this->type && empty($element->readonly) && empty($element->levelReadOnly) && ( empty($element->onlynew)
			 || ( $this->_newEntity && $element->onlynew==1 )
			 || ( !$this->_newEntity && $element->onlynew==2 ))){

				if( $highLevel){
					$field->content=$element->levelReadOnly . ' ' . WText::t('1242048721QUXH');
					$htmlStatus=true;
				}else{
				...
				if( $highLevel){
					...
					$htmlStatus=$field->displayShow();
																$keyArey=( !empty($element->area) ? $element->area : $idLabel );
...
class WForms_standard extends WElement {
	...
	public static function &frame(&$element,&$params,$reset=false,$layoutSubType=0){
		...
		$index=$element->frame . $element->parent;
				if( !isset($frame[$index])){
			...
			$data->params=$params;
			$frame[$index]=WPage::renderBluePrint( 'frame', $data );

		}
				 $frame[$index]->_data=$element;
	...
	private function _setMSG($wid,$type,$value=''){
		...
		if( !empty($messageSpecific) && !isset($jscript['js']['onThefly']['msg'][$type."_" . $this->idLabel] )) WPage::storeScript( "jCore.msg['".$type."_" . $this->idLabel ."']=\"" . $messageSpecific . "\";", 'js', 'msg' );

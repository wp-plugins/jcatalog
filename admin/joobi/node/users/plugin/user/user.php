...
class Users_User_plugin extends WPlugin {
	...
	function onUserAfterSave($user,$isnew,$success,$msg){
		...
		$status=$usersAddon->syncUser( $user, $isnew, $success, $msg );
	...
	function onLoginUser($user,$options)	{
		...
		if( !empty($ip)){
			...
			if( !empty($ip)){
				$usersM->setVal( 'ip', $ip, 0, null, 'ip' );
								$ipTrackerC=WClass::get( 'iptracker.lookup', null, 'class', false );
				if( !empty($ipTrackerC)){
		...
		if( WRoles::isNotAdmin()){
						...
						$basketClearC=WClass::get( 'basket.clear', null, 'class', false );

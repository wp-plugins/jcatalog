	...
	public function completeInformation(&$user){
		...
		if( is_object($ipLookupC)){
			...
			if( empty($localization)){
								$user->timezone=JOOBI_TIME_OFFSET;
				$user->_tzExact=false;															}else{
								$user->timezone=( isset($localization->country->timezone)) ? $localization->country->timezone : JOOBI_TIME_OFFSET;
				$user->_tzExact=false;
			}
		}else{
			$user->_tzExact=true;

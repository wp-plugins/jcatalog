...
class Files_Media_class extends WClasses {
			...
			if ( $myFileInfoO->type != 'url' && $mediatypes->inValues( $myFileInfoO->type ) ) {
				...
				if (in_array( $extensionType, array( 'jpg', 'jpeg', 'gif', 'png', 'bmp' ) ) ){
					...
					$HTML .= $this->image();
								...
								elseif ( in_array( $extensionType, array( 'mp3', 'wav', 'aiff', 'wma' ) ) ){
					$this->fileType = 'audio';
					$HTML .= $this->_audio();
				}
					...
					if ( $myFileInfoO->type != 'url' ) {
						...
						$myLink .= $this->convertPath( $myFileInfoO->path );
						...
						if ( WRoles::isAdmin( 'manager' ) ) $HTML .= ' ' . $myFileInfoO->name . '.' . $myFileInfoO->type;
	...
	function convertPath($path='',$type='hdd') {
		if ($type=='hdd'){
			$type = DS;
		}elseif ($type=='url'){
			$type = '/';

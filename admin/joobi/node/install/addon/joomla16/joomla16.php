...
class Install_Joomla16_addon {	
	public $apiVersion = 'joomla16';
	...
	public function module() {
		...
		$sql->insert();
				...
				if ( !empty($sql->id) ) {
			$model= WTable::get('#__modules_menu','','moduleid,menuid');
...
/**
* @link joobi.co
* @copyright Copyright (C) 2006-'.date('Y').' Joobi Limited All rights reserved.
...
';
	...
	public function setXmlAndMagicFile($type,$extension_name) {
		...
		switch ( $type ) {
				...
				if ( $this->generate ) {
					...
					$this->xmldata[0]['children'][] = array('nodename'=>'uninstallfile','attributes'=>array(),'nodevalue'=>$name.'_uninstall.php');
					...
					foreach($this->content[0]['children'] as $k => $mainnode){
						...
						}elseif ($mainnode['nodename'] == 'uninstallfile'){
							$this->content[0]['children'][$k]['nodevalue'] = $name.'_uninstall.php';
							$uninstallfile_found = true;
						}					}
					if (!$uninstallfile_found){
						$this->content[0]['children'][] = array('nodename'=>'uninstallfile','attributes'=>array(),'nodevalue'=>$name.'_uninstall.php');
					}				}
				...
				$this->path_uninstall = $same.'_uninstall.php';
				$this->path_install = $same.'_install.php';
				break;
							case 'module':
	...
	function menuExist($name,$menuType) {
				...
				$CMSMenuTypeTable = WTable::get('#__menu_types','','id');
	...
	function addMenu($vars=null,$parent=1,$option=null) {
		...
		$client_id = ( !isset($menuM->client_id) ? 1: $menuM->client_id );

				$menuM->whereE( 'client_id', $client_id );
		$menuM->whereE( 'parent_id', $this->parent );
		$menuM->whereE( 'alias', $menuM->alias );
		$menuM->whereE( 'language', '*' );
		$existingID = $menuM->load( 'lr', 'id');
		...
		if ( !isset($menuM->ordering) ) $menuM->ordering = -2147483648;
	...
	protected function _insertExtensionDB($name,$namekey,$type='component',$folder=null) {
		...
		if ( !$exist ) {
			...
			$myManifest['copyright'] = '(C) 2006-'.date('Y').' Joobi Limited. All rights reserved.';
			$myManifest['authorEmail'] = 'support@joobi.co';
			$myManifest['authorUrl'] = 'http://www.joobi.co';
	...
	protected function getModuleFolder($name){
 	...
 	public function generateXml(&$ext,$type) {
		...
		if ( isset($this->tables) ) {
			$lgid = 0;
			if ( array_key_exists( 'joobi_languages', $this->tables ) ) {
				foreach($this->tables['joobi_languages'] as $row){
					if ($row->code == 'en'){
						$lgid=$row->lgid;
						break;
					}				}			}
...
href="https://www.joobi.co/r.php?l=documentation" target="_blank">Read
Tutorials</a><br>
</td>
...
<br>
<a style="color: white;" href="https://www.joobi.co/r.php?l=support" target="_blank">Submit a Ticket</a><br>
</td>
</tr>
<tr>
...
<br>
<a style="color: white;" href="https://www.joobi.co/r.php?l=forum" target="_blank">Ask a Question</a><br>
</td>
...
<br>
<a style="color: white;" href="https://www.joobi.co/r.php?l=support" target="_blank">Talk to Us</a><br>
</td>
</tr>
</tbody>
</table>
';
		...
		if ( empty( $options['copyright'] ) ) $options['copyright'] = '(C) 2006-'.date('Y').' Joobi Limited. All rights reserved.';
		if ( empty($options['authorEmail']) ) $options['authorEmail'] = 'wecare@joobi.co';
		if ( empty($options['authorUrl']) ) $options['authorUrl'] = 'http://www.joobi.co';
...
class Install_Joomla15_addon extends Install_Joomla16_addon {
}

...
self::logMessage( 'function updatePref :' . $install_status );
self::logMessage( debug_backtrace( DEBUG_BACKTRACE_IGNORE_ARGS ) );
...
self::logMessage( 'function updatePref : end' );
	}
...
self::logMessage( 'All packages extracted', 'install', false, 1, true, true);
		...
		$this->mode='copy';
		$this->updatePref($_SESSION['joobi']['install_status']);
...
self::logMessage( 'installPackages function  finish_' . $this->mode , 'install', false, 0, false );
	...
	function addUpdateDependencies() {
		...
		$appsDepM = WTable::get( 'extension_dependency', 'main_library', 'wid,ref_wid' );
...
self::logMessage( 'overwrite entry point file ' . $entry_file . ' using cms addon file ' . $cms_addon, 'install', false, 0, false  );
				...
				switch( $className ) {
					case 'joomla16':
						$fileHandler->write( $entry_file, Install_Joomla16_addon::magicFile(), 'force' );
						break;
					default:
...
self::logMessage( 'skipping entry point overwrite, no cms addon file at ' . $cms_addon, 'install', false, 0, false  );
	...
	private function _getListOfPackages($already=false) {
		...
		$sentData->database_name = JOOBI_DB_TYPE;
		if ( defined('JOOBI_DB_VERSION') ) $sentData->database_version = JOOBI_DB_VERSION;
		...
		if ( empty($sentData->token) ) {
						$status = $appsInfoC->getPossibleCode( $getPossibleCode );
			if ( $status === false ) return $this->_finishWithError( WText::t('1427652813TJJQ') );
...
self::logMessage( 'Package ' .$package. ' downloaded successfully', 'install', false, 0, false );
	...
	private function _finishWithError($message) {
		...
		$LINK = '<a target="_blank" href="https://www.joobi.co/r.php?l=support">' . WText::t('1389701766DFGL') . '</a>';

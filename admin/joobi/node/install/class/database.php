	...
	public function import($file,$basedir='',$compat=false,$update=null) {
		...
		if ( $basedir === true ) {
			...
			if ( $query != '' && substr( $query, 0, 1 ) != '#' ) {
				...
				$sqlInstanceQ =  WQuery::getDBConnector( JOOBI_DB_TYPE );
	...
	public function importFile($file,$split=true) {
		...
		try {
			...
			$handler = WClass::get( 'library.sql' );
				...
				$this->importQueries( $handler->splitSql( $data, ';', null, false ) );
			...
			if ( count($array) >0 ) {
				...
				foreach( $array as $query ) {
					...
					if ( !empty($query) ) {
						...
						$query = preg_replace( '/#database_[0-9]{0,4}#/', $this->getDBName(), str_replace( '#__', $this->getTablePrefix(), $query) );

...
class Apps_CorePlan_listing extends WListings_default{
	...
	function create(){
		...
		if( $this->getValue( 'publish' )){
			...
			if( $namekey !=JOOBI_MAIN_APP . '.application' && $namekey !='jstore.application' && $namekey !='jcatalog.application'){
				if( $maintenance > 2506000){
					...
					$this->content .='<small><span style="color:' . $color . ';">' . WApplication::date( WTools::dateFormat( 'date' ), $maintenance ) . '</span></small>';
				}
			}
		}else{
			return false;

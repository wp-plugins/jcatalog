...
class Apps_Refresh_class extends WClasses {
	...
	private $refreshTables=array('apps','library.languages','appstrans','apps.version','apps.level','apps.leveltrans','apps.dependency','apps.info');
	...
	public function checkNewDistributionServer($report=true){
		...
		$NotYetUpdatedA=$appsM->load( 'lra', 'wid' );
				...
				if( !empty( $NotYetUpdatedA )) return true;
						...
						foreach( $GoodDAta as $oneDistribSite){
							...
							switch( $platformA[1]){
								case 'beta':
									...
									if( $lastSet < ( time() - 2592000 )){											$installPref->updatePref( 'distrib_website_beta', $oneDistribSite->url );

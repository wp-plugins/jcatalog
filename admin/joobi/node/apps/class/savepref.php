	...
	function appsSave(&$preferencesA){
		...
		if( isset($preferencesA['users.node']['framework_fe'])){
			...
			if( 99==$preferencesA['apps.node']['distribserver']){
				...
				if( $result){
					$message->userS('1298294128HPXJ',array('$HOMESITE'=>$HOMESITE));
				}else{
					$message->userE('1298294128HPXK',array('$HOMESITE'=>$HOMESITE));
				}
			}
			...
			$message=WMessage::get();
			$message->userN('1303354916DPMU',array('$ENABLED'=>$ENABLED));

...
class Apps_Info_class extends WClasses {
	...
	public function possibleUpdate($eid=0,$allowLocalHost=true){
		static $status=null;
...
$SKIP=false;
			...
			$appsQueryC=WClass::get( 'apps.query' );
				...
				if( empty( $eid )){
					...
					if( in_array( $namekeyApp, array( JOOBI_MAIN_APP . '.application', 'jstore.application', 'jcatalog.application' ))){
						return true;
					}
										$app=new stdClass;
					$app->wid=$eid;
				...
				if( !empty( $allPossibleUpdateA )){
					...
					foreach( $allPossibleUpdateA as $oneApp){
						...
						$this->_fileName=JOOBI_DS_NODE . $folder . DS . 'li'.'nks.'.'php'; 							if( @file_exists($this->_fileName)){
							
							$hasFile=true;

														$token=$appsQueryC->getSweetInfo( $this->_fileName, $oneApp->wid, 'token' );
	...
	public function getPossibleCode($kind2Get=true,$return='token'){

		$filename=JOOBI_DS_NODE . 'main' . DS . 'li'.'nks.'.'php'; 			if( @file_exists($filename)){							$appsQueryC=WClass::get( 'apps.query' );
						$wid=WExtension::get( JOOBI_MAIN_APP . '.application', 'wid' );
			return $appsQueryC->getSweetInfo( $filename, $wid, $return );

		}else{				$appsQueryC=WClass::get( 'apps.query' );

			if( is_numeric($kind2Get)){
								$folder=Wextension::get( $kind2Get, 'folder' );
				$filename=JOOBI_DS_NODE . $folder . DS . 'li'.'nks.'.'php'; 					return $appsQueryC->getSweetInfo( $filename, $kind2Get, $return );

			}elseif( $kind2Get !='notOne'){
				...
				if( !empty( $allPossibleUpdateA )){
					...
					foreach( $allPossibleUpdateA as $oneApp){
						...
						$filename=JOOBI_DS_NODE . $folder . DS . 'li'.'nks.'.'php'; 							if( @file_exists($filename)){
														...
														$token=$appsQueryC->getSweetInfo( $filename, $oneApp->wid, $return );
			...
			if( !empty($token)){
				...
				if( 'js'.'to'.'re.ap'.'plica'.'tion'==$kind2Get || 'jce'.'nter.ap'.'plica'.'tion'==$kind2Get){
										return true;
				}
				static $firstDemoInstall=null;

				if( !isset($firstDemoInstall)){
										$mainAppID=WExtension::get( JOOBI_MAIN_APP . '.application', 'wid' );
					$appsM=WModel::get( 'apps' );
					$appsM->whereE( 'wid', $mainAppID );
					$firstDemoInstall=$appsM->load( 'lr', 'created' );
...
return true;
		...
		$extensionUserInfosM->setVal( 'maintenance', $maintenance );
		...
		if( !$statusFile ) return false;
				...
				$appsDependencyM=WModel::get( 'apps.dependency' );
		...
		$appsDependencyM->makeLJ( 'apps.dependency', 'wid','wid',1,2 );
		$appsDependencyM->makeLJ( 'apps', 'ref_wid','wid',2,3 );
	...
	public function getValidToken($onlyPlan=true){
	...
	private function _checkValidToken($token,$showErrorMessage=true,$showSuccessMessage=false){
		...
		$message=WMessage::get();
			...
			switch( $result){
				case 'tokenValid':
					$message->userS('1389701736EXGW');
					return true;
				case 'tokenExpired':
					$message->userE('1389701736EXGX');
										return false;
				case 'tokenNotExist':
					$message->userE('1389701736EXGY');
					return false;
				case 'tokenMismatch':
					$message->userE('1389701736EXGZ');
					return false;
				case 'tokenNotPossibleResult':
					$message->userE('1389293010DETJ');
					return false;
				default:
					$message->userE('1415107844LLSU');
					return false;
			}
		}
		return false;
	...
	private function _checkReason(){
		...
		switch( $this->_reason){
			case 'tokenExpired':
					...
					$fileS=WGet::file();
					$fileS->displayMessage( false );
					$fileS->delete( $this->_fileName );
					$this->_fileName='';

					if( !empty($this->_token)){
												$appsUserinfosM=WModel::get( 'apps.userinfos' );
						$appsUserinfosM->whereE( 'token', $this->_token );
						$appsUserinfosM->delete();
	...
	private function _possibleUpdateA(){
		...
		if( !isset($allPossibleUpdateA)){
			$appsUserinfosM=WModel::get( 'apps.userinfos' );
$appsUserinfosM->remember( 'possibleUpdateA', true, 'Extension' );

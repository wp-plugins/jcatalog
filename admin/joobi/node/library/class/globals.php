...
abstract class WGlobals {
    ...
    if( ! WGlobals::_static( 3, $location)) WGlobals::_static( 1, $location );
        if( @array_key_exists($name,$data) && !$overwrite ) return false;
	...
	if( !empty($propertyName)){
		if(  isset( $_SESSION[$domain]->$propertyName)){
			 			return WGlobals::filter( $_SESSION[$domain]->$propertyName );
		} else return $default;
	}else{
		if( isset($_SESSION[$domain])){
						return WGlobals::filter( $_SESSION[$domain] );
		}else{
			return $default;
		}	}
  }
    ...
    return ( $route ) ? WPage::routeURL( $url) : $url;
	...
	public static function setReturnId($url){
    WGlobals::set('returnid', base64_encode($url));
  }
	...
	public static function getUserState($token,$name,$default=null,$type='',$getInTruc=false){
		...
		if( false===$getInTruc){
			...
			$trucs=WGlobals::get( 'trucs', null, '', 'array' );
			$new=$trucs['x'][$name];
		}else{
			$new=WGlobals::get( $name, null, '', $type );
		}
	...
	public static function getEntireSuperGlobal($src=''){	  	$data=null;
	...
	public static function checkCandy($level,$dontHave=false){			return ( $dontHave ? false : true );
	}
	...
	public static function getCandy($extension='zzzzz'){
		return 50;			return (( isset(self::$levelAccess[$extension])) ? self::$levelAccess[$extension] : 0 );
	}
	...
	switch( strtolower($type)){
      ...
      case 'namekey':	      	$data=str_replace( array( '|', ' ' ), '_', $data );
        return (string)preg_replace( '/[^a-zA-Z0-9_.\-]+/', '', $data );
      case 'email':
        if( preg_match('/^[a-zA-Z0-9]([.]?([a-zA-Z0-9_-]+))*?@([a-zA-Z0-9\-_]+\.)+[a-zA-Z]{2,4}$/', $data )==false ) return '';
...
echo '<br>TYpe: ' . gettype($data);
echo debugB( 78787878 );
      		return $data;
      	}
      ...
      default:
        ...
        if( is_bool($data)){	        	return $data;
                ...
                return WGlobals::filter( $data, 'string', $default, '', '', $check );
	...
	if(is_array($data)){
    	$new_data=array();
          ...
          {
            $new_data[ WGlobals::filter( $key,'',null,'','',$check ) ]=WGlobals::filter( $val,'',null,'','',$check );
          }
          $data=$new_data;
    }else{
   		$mess=WMessage::get();
    ...
    $name=( substr($name,0,1)=='_') ? strtoupper($name) : '_' . strtoupper($name);
	...
	private static function _static($type,$var='',$array='',$value=null){
	  	static $vars;
if( 'opidtest'==$var ) debug( 898911, $type );
	    switch($type){
	      	      case 0:
	        $vars=array();
	        break;
    ...
    if( is_object($data)){
		...
		$attributes=get_object_vars( $data );
		foreach($attributes as $key=> $val){
			$key=WGlobals::filter($key,'',null,'','',$check);
			$new_object->$key=WGlobals::filter( $val, $type, $default, '', '', $check );
		}		$data=$new_object;
    }else{
		$data=$default;
    }
  ...
  private static function _storeEID($double=false,$set=null){
    ...
    if( isset($set)){
      ...
      $pkey=( !empty($trucs['s']['pkey']) ? $trucs['s']['pkey'] : '' );
      $sid=( !empty($trucs['s']['mid']) ? $trucs['s']['mid'] : 0 );
    ...
    if( empty($eid) || !empty($modelID)){
      ...
      $eid=WGlobals::get( 'eid', null, '', 'string' );

      if( empty( $eid)){

      	$trucs=WGlobals::get( 'trucs' );

      	if( !empty($trucs['s']['pkey'])){
			$pkey=$trucs['s']['pkey'];
		}else{
			$pkey='';
		}
      	$sid=( !empty($trucs['s']['mid'])) ? $trucs['s']['mid'] : '';
      	if( !empty($modelID) && !empty($sid) && $modelID !=$sid ) $sid=0;
        if( !empty($pkey)){
        	if( is_array($pkey)){
        		$eid=array();
        		foreach( $pkey as $onePKEY){
        			$myEID=WGlobals::get($onePKEY.'_'.$sid, null );
        			if( !empty($myEID)) $eid[]=$myEID;
        		}        	}else{
        		$eid=WGlobals::get( $pkey . '_' . $sid, null );
        	}
			if( empty($eid)){
				if( is_array($pkey)){
					$eid=array();
					foreach( $pkey as $uniquePK){
						$eid[]=( isset($trucs[$sid][$uniquePK])) ? $trucs[$sid][$uniquePK] : 0;
					}				}else{
					$eid=( isset($trucs[$sid][$pkey])) ? $trucs[$sid][$pkey] : 0;
				}
			}        }elseif( !empty($sid)){	          
			$PKs=WModel::get( $sid, 'pkey' );
			if( strpos( $PKs, ',')===false ){					$eid=WGlobals::get( $pkey.'_'.$sid, null );
			}else{
				$expolodePK=explode( ',', $PKs );
				if( !empty($expolodePK)){
					foreach( $expolodePK as $onePK){
						if( isset($trucs[$sid][$onePK])) $eid[$onePK]=(string)$trucs[$sid][$onePK];
					}				}else{
					$eid=0;
				}			}        }else{
        	$eid=0;

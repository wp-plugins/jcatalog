...
class WTable extends WQuery {
	...
	function __construct($tablename='',$dbname='',$pkey=null,$params=null,$tablePrefix=null){
	  	...
	  	if( !empty($tablename) && is_numeric($tablename)){
			...
			if( 'wordpress'==JOOBI_FRAMEWORK_TYPE ) WTable::$addDBName=false;
	    ...
	    if( !empty($tablename) && !is_numeric($tablename)){
	    	$this->_infos->tablename=str_replace( '#__', $this->_infos->tableprefix, $tablename );
	    }
		...
		if( empty($this->_infos->addon)) $this->_infos->addon=JOOBI_DB_TYPE;
	...
	public function makeLJ($tablename,$dbname='',$cond1='',$cond2='',$as1=0,$as2=0,$firstOperator=''){
	...
	public function operator($op='OR'){
		...
		$this->_operators[count($this->_whereValues)]=$object;
	...
	public static function getAvailableLanguages(){
		...
		$languagesM->whereE( 'publish', 1 );
		$languagesM->setLimit( 500 );
		$result=$languagesM->load( 'lra', array('lgid'));
      ...
      if( !empty($this->_selectA)){
		$select=array();
      	...
      	foreach( $this->_selectA as $selectKey=> $oneSelect){
            ...
            if( !empty($oneSelect->special)){
	            	            $select[]=$this->_makeC( $oneSelect->map, $oneSelect->as1, $oneSelect->special ) . $as;
            }else{
	            	            $select[]=$this->_makeC( $oneSelect->map, $oneSelect->as1 ) . $as;
            }
	    ...
	    if( !empty($this->_groupby)){
	      ...
	      foreach( $this->_groupby as $grouby){
	        $myGrouBy[]=$this->_makeC($grouby->champ,$grouby->asi);
	      ...
	      foreach( $this->_orderby as $myOrder){
	      	...
	      	if( !empty($myOrder->useAlias)){
		        ...
		        $orderby[]=$this->_makeC( $myOrder->champ, $myOrder->asi ).' '. $checkedOrderBy;
	        }
	      }
	    	...
	    	if( !empty($termsSeachedIn) || !empty($termsSeachedInAddedA)){
	    		...
	    		if( !empty( $termsSeachedInAddedA )){
	    			foreach( $termsSeachedInAddedA as $addObjsA){
	    				$searchTempAddedA[]=$this->_makeC( $addObjsA->map, $addObjsA->asi ) . $addObjsA->searched;
	    			}	    		}
	    		if( !empty($termsSeachedIn)){
		    		foreach( $termsSeachedIn as $ssTerm=> $ssObjsA){
						$searchTemp=array();
		    			foreach( $ssObjsA as $ssObj){
				        	if( PLIBRARY_NODE_LWRSEARCH ) $searchTemp[]=' LOWER( ' . $this->_makeC( $ssObj->map, $ssObj->asi) . ' ) ' . $ssObj->searched;
		    				else $searchTemp[]=$this->_makeC( $ssObj->map, $ssObj->asi) . $ssObj->searched;
		    			}
	...
	private function _makeWhereDefault($myWhere,$noQuotes=false){
  	...
  	if( isset($myWhere->asi2)){	  		if( $this->_identify){
		...
		return $this->_makeC($myWhere->champ,$myWhere->asi1).$myWhere->cond. ' ' . $value;
	...
	private function _makeWhereIn($myWhere){
        ...
        $column=' '.$this->_makeC($myWhere->champ,$myWhere->asi).' ';
	...
	private function _makeWhereIsNull($myWhere){
		return $this->_makeC( $myWhere->champ, $myWhere->asi ) . (( $myWhere->condition ) ? ' IS NULL ' : ' IS NOT NULL ' );
	}
    ...
    if( !empty($this->_update)){
        ...
        foreach( $this->_update as $updateObj){
          ...
          if( isset($updateObj->valueAS)){
            $update[]=$column .'=' . $this->_makeC( $updateObj->value, $updateObj->valueAS );
          }else{
          ...
          if( is_array($v) or is_object($v) or $k[0]=='_' or  (strtoupper($k[0])===$k[0])  ) continue;
			if( $k=='ip'){
				if( !empty($v)){
				    $update[]=$this->_makeC( $k ) .'='. $this->_db->insertInetAton( $this->valueQuote( $v ));
				}else{
					continue;
				}			}else{
			  $update[]=$this->_makeC( $k ) .'='.$this->valueQuote($v);
			}
        }
    }

...
class Library_debug_class {
	...
	function makeTDHeader($type,$header,$indent=0){
		...
		if( is_array($var)){
			...
			foreach($var as $key=> $value){
				...
				if( $this->_HTMLLook){
					...
					$html .=$this->makeTDHeader( "array", $key );

				}else{

					$html .=$this->_addIndent( $indent ) . '[' . $this->makeTDHeader( "array", $key, 0 ) .']';
			...
			if( $this->_HTMLLook){
				...
				$html .="<tr><td>".$this->error("array").$this->closeTDRow();
...
$haveModel=false;
		...
		if( is_object($var)){
			...
			foreach($arrObjVars as $key=> $value){
				...
				if( $this->_HTMLLook){
					...
					$html .=$this->makeTDHeader( "object", $key );

				}else{

					$html .=$this->_addIndent( $indent ) . '(' . $this->makeTDHeader( "object", $key, 0 ) .')';
			...
			$html .="<tr><td>".$this->error("object").$this->closeTDRow();
	...
	function varIsResource($var,$indent=0){
		...
		switch(get_resource_type($var)){
			...
			default:
				...
				$html .=get_resource_type($var).$this->closeTDRow();

				break;

		}

		$html .=$this->closeTDRow()."</table>\n";
	...
	function varIsXmlResource($var){
		...
		$html .=$this->makeTDHeader("xml","xmlRoot");
		...
		if($bFile){
			...
			if(!is_string($var)){
				...
				$html .=$this->error("xml").$this->closeTDRow()."</table>\n";
	...
	function xmlStartElement($parser,$name,$attribs){
		...
		$this->xmlSData[$this->xmlCount].='$html .=$this->makeTDHeader("xml","xmlName");';

		$this->xmlSData[$this->xmlCount].='$html .="<strong>'.$this->xmlName[$this->xmlCount].'</strong>".$this->closeTDRow();';

		$this->xmlSData[$this->xmlCount].='$html .=$this->makeTDHeader("xml","xmlAttributes");';
		...
		$this->xmlSData[$this->xmlCount].='$html .=$this->closeTDRow();';
	...
	function xmlEndElement($parser,$name){
		...
		for($i=0;$i<$this->xmlCount;$i++){
			...
			$html .=$this->makeTDHeader("xml","xmlText");

			$html .=(!empty($this->xmlCData[$i])) ? $this->xmlCData[$i] : "&nbsp;";

			$html .=$this->closeTDRow();

			$html .=$this->makeTDHeader("xml","xmlComment");

			$html .=(!empty($this->xmlDData[$i])) ? $this->xmlDData[$i] : "&nbsp;";

			$html .=$this->closeTDRow();

			$html .=$this->makeTDHeader("xml","xmlChildren");
	...
	function varIsGDResource($var){
		...
		echo 'My image X 88776655<br/>';

		print_r(imagesx($var));

		echo 'My image Y 88776655<br/>';

		print_r(imagesy($var));

		echo 'My image color 88776655<br/>';

		print_r(imagecolorstotal($var));

		$html='';

		$html .=$this->makeTableHeader("resource","gd",2);

		$html .=$this->makeTDHeader("resource","Width");

		$html .=imagesx($var).$this->closeTDRow();

		$html .=$this->makeTDHeader("resource","Height");

		$html .=imagesy($var).$this->closeTDRow();

		$html .=$this->makeTDHeader("resource","Colors");

		$html .=imagecolorstotal($var).$this->closeTDRow();

...
class WMessage {	
	...
	private function codeM($mess,$variable=array(),$type='warning'){
			...
			if(defined('JOOBI_INSTALLING') || JOOBI_DEBUGCMS || (defined('PLIBRARY_NODE_DBGERR') && PLIBRARY_NODE_DBGERR )){
				$this->addM( $mess, $type, $variable, true );
			}else{
				if( WRole::hasRole( 'manager' )){
					switch($type){
						case 'warning': $functionName='userW';
							break;
						case 'notice': $functionName='userN';
							break;
						case 'success': $functionName='userS';
							break;
						default: $functionName='userE';
							break;
					}
					$this->$functionName( 'An Error Occurred! You can display the real error message only for your user account by going in the back end of your website and turning on the debug mode in the Genral Preferences.' );
	...
	private function historyM($mess='',$variable=array(),$redirectInController=false,$typeError='E'){
		...
		if( !empty($mess)){
			if( in_array( $typeError, array('W', 'E', 'N', 'S'))){
				...
				WGlobals::setSession( 'historyRedirect', 'trucs', $trucs );
	...
	public function cleanBuffer($file=''){
	...
	public static function showLine($entries=1,$html=true){
		$mess='';
				...
				if( defined( 'PLIBRARY_NODE_ERRORSHOWLINE' ) && PLIBRARY_NODE_ERRORSHOWLINE){
	...
	public static function log($message,$location='system-logs',$deleteBefore=false,$entries=1,$showTime=true,$showMemory=false){
		...
		if( $deleteBefore && $fileExist && !$filehandler->delete( $file )) return false;
			...
			$beginning='#<'.'?php die(\'Direct Access To Log Not Permitted\'); ?>';
		}else{
			$beginning='';
		}

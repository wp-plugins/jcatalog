...
class WQuery extends WObj {
	...
	function __construct($params=null,$setupDB=true){
		...
		if( !empty($this->_infos->tablename)) $this->_infos->tablename=str_replace( '#__', $this->_infos->tableprefix, $this->_infos->tablename );
	...
	public function setDBConnector(){
		if( !isset($this->_db)) $this->_db=WQuery::getDBConnector( JOOBI_DB_TYPE );
	}
  ...
  function deleteDB($database){
	...
	public function getTableName($addPrefix=true){
		...
		if( !empty($this->_infos->tablename)){
			...
			return $return . substr( $tableName, $prefixLen );
		}else{
...
protected function _resultHandle($query,$result=false){
	 ...
	 || (defined( 'PLIBRARY_NODE_DBGQRY') && PLIBRARY_NODE_DBGQRY )
      ...
      if( ! $logQueries){
	      if( defined( 'PLIBRARY_NODE_LOGQRY') && PLIBRARY_NODE_LOGQRY){
	      	$logQueries=PLIBRARY_NODE_LOGQRY;
	      }elseif( ! IS_ADMIN && defined( 'PLIBRARY_NODE_LOGQRYGUEST') && PLIBRARY_NODE_LOGQRYGUEST){
	      	$isRegsitered=WUser::isRegistered();
	      	$logQueries=(!$isRegsitered) ? PLIBRARY_NODE_LOGQRYGUEST : $this->_log;
	      }      }
	...
	if( defined('PLIBRARY_NODE_DBGQRY') && PLIBRARY_NODE_DBGQRY && !empty($uid)){
		$show=true;
	}elseif( defined('PLIBRARY_NODE_DBGQRYGUEST') && PLIBRARY_NODE_DBGQRYGUEST && ! IS_ADMIN && $uid==0){
						$show=true;
	}
		...
		$QueryExactTime=round((microtime(true) - @$this->_microtimeExactQuery ) * 1000, 2 );
	...
	private function _HTMLFormatSQL($query,$color='none',$logFile=false){
  		...
  		$replace=array( 'LEFT JOIN', 'WHERE', 'AND', 'OR', 'FROM', 'ORDER BY', 'GROUP BY' );
  		$replaceBy=array( $separator.'LEFT JOIN', $separator.'WHERE', $separator.'AND', $separator. 'OR', $separator .'FROM', $separator .'ORDER BY', $separator .'GROUP BY' );

  		$query=str_replace( $replace, $replaceBy, $query );

  		if( $color !='none' ) $query='<span style="color: '.$color.';">'.$query.'</span>';

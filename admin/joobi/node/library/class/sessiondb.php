...
class Library_Sessiondb_class extends Library_Sessionparent_class {
	...
	public function read($id){
		...
		$sessionM->select( 'ip', 0, 'realip', 'ip' );
		$sessionInfo=$sessionM->load( 'o', 'data' );

		if( !empty($sessionInfo)){
			$userSessionC=WUser::session();
			$myIP=$userSessionC->getIP();
			if( !empty($sessionInfo->realip) && '0.0.0.0' !=$sessionInfo->realip && $sessionInfo->realip !=$myIP){

WMessage::log( 'user switch IP from : ' . $sessionInfo->realip . '  to  : ' . $myIP, 'switch_ip' );

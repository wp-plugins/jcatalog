	...
	public function getdata($id,$className,$cachingType='cache',$mandatoryStatic=true,$stringID=false,$functionName='',$params=null,$showMessage=true){
		...
		if( is_numeric($id) || $stringID){
			...
			if( !empty($data)){									if( !empty( $data->namekey )){
				...
				if( $mandatoryStatic ) $localData[$className]['d'][$id]=$data;
				$registry[$cachingType]->setCache( $className, 'd-' . $id, $data );
		...
		if( $mandatoryStatic && !empty($localData[$className]['d'][$returnId])) return @$localData[$className]['d'][$returnId];
	...
	public function resetCache($domain=null,$id=null,$type='cache'){
		...
		if( empty($id)){
			if( empty($domain)){
				 ...
				 'Theme', 'Table', 'Events', 'Model_mailing_node', 'Controller', 'Action', 'Language', 'Tag', 'Preference', 'Translation', 'Menus' );

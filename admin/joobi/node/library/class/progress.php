	...
	public function get($name){
		...
		$this->_maxExecTime=self::$_instanceA[$name]->_maxExecTime;
			...
			$this->codeE( 'No steps defined' );
				...
				self::$_instanceA[$name]->_fileName=$name . '_steps.stp';
...
WMessage::log( $this, 'install-progress-max-increment' );
	...
	protected function addParams($property,$value){
		...
		if( !isset($this->_progressO->params)) $this->_progressO->params=new stdClass;
		$this->_progressO->params->$property=$value;
	...
	protected function readParams($property,$default=null){
		...
		if( isset($this->_progressO->params->$property)) return $this->_progressO->params->$property;
		else return $default;

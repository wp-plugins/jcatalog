...
class WModel_Loadinfo {
	...
	public function getModelInformation($modelName,$eid,$property='data'){
		...
		if( $modelMemoryM->hasMemory()){
			$returnedA=$modelMemoryM->loadMemory( $eid, null, false );
		}else{
			$PK='';
			$returnedA=$this->_loadDataFromModel( $modelID, $modelName, $eid, $multipleValues, $PK );
		}		if( $multipleValues){
	...
	private function _formatReturnObject($object,$property){
		...
		if( 'data'==$property || empty($property)) return $object;
	...
	private function _loadDataFromModel($modelID,$modelName,$eid,&$multipleValues,&$PK){

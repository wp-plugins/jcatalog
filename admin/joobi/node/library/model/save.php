...
class WModel_Save {
	...
	public function save($validate=true,&$localObj){				if( !$validate ) $localObj->_validate=false;
		...
		$status=true;
			...
			if( !empty($localObj->wfiles)){
				...
				foreach( $localObj->wfiles as $arrKey=> $arrVal){
						...
						if( !empty($arrVal2->name)) $filesIDs[$arrKey][]=$this->_saveFiles( $arrVal2, $localObj );
			...
			if( $localObj->getType()==30){		
				...
				foreach( $filesIDs as $filesIDsKey=> $filesIDsVal){
					...
					if( !is_object( $childModel ) || empty($childModel)){
						$mess=WMessage::get();
					...
					$localObj->_new=true;
										...
										if( isset($localObj->$mychildPK[0])) $this->_saveChild( $childModel, $childName, false, $localObj->$mychildPK[0], $strangeArray, array($filesIDsKey), $localObj );

				}
								if( !empty($filesIDs)){
					$i=0;
					foreach( $filesIDs as $filesIDsKey=> $filesIDsVal){
						$localObj->$filesIDsKey[$i]=$filesIDsVal;
						$i++;
					}				}
								return true;

			}else{					if( !empty($filesIDs)){
				...
				foreach( $localObj->f as $fileType=> $content){
					...
					if( !empty($trucs['x'])) $fileObject->x=$trucs['x'];
					...
					if( isset($oneMulSelectMap)){
						foreach( $oneMulSelectMap as $oneMultVal){
							if( isset($localObj->$oneMultVal) && is_array( $localObj->$oneMultVal )){
																								$localObj->$oneMultVal='|_|' . implode( '|_|', $localObj->$oneMultVal ) . '|_|';
							}						}					}				}			}
			...
			if( !$localObj->_new && !$localObj->multiplePK()){
				...
				if( !empty($constraints)){
					if( !$this->_verifyData( $constraints, $messageConstraints, $localObj->_new, $localObj )){
						$mess=WMessage::get();
						$mess->historyE( $messageConstraints );
					}				}			}
							...
							if( isset($model->_samePrimaryKey) && $model->_samePrimaryKey){										if( !empty($localObj->$pKey)) $model->_forceInsert=true;
								...
								if( !empty($arrayInValue)){
									...
									sort($arrayInValue);	
										...
										if( !in_array( $arrayInValue[0], $FileinfoA ) && empty($localObj->$childVal->$arrayInValue[0])){
																																	continue;
										}
	...
	public function saveFileLib(&$localObj,$source='',$destination='',$validate=true,$property=null){
	...
	public function copy($eid=null,&$localObj){
		...
		unset($localObj->$pkey);  				$localObj->noVerify();

				$namekeyEID=array();
		$this->_checkUK( true, true, $localObj, $namekeyEID );

				$this->_checkUKForChildModel( true, true, $localObj, $namekeyEID );
	...
	public function copyAll($eid=0,&$localObj){
		...
		if( @is_array( $eid )){
			...
			$obj=$this->_getChildsCopy( $localObj->getTableId(), $eid, $parent, $namekeyEID );
	...
	private function _getChildsCopy($dbtid,$eid,&$parent,&$namekeyEID){
		...
		if( !isset($storeFK[$dbtid])){
			...
			$sql->whereSearch( 'namekey', '.view', 2 );
		...
		$childObj=new stdClass;
			...
			if( $sql->multiplePK()){
				...
				foreach( $childKeys as $ckey=>$childKey){	
					...
					$myobj=$this->_getChildsCopy( $makeInstanceChildM->getTableId(), $eidValue, $parent, $namekeyEID );
	...
	private function _createWFile(&$localObj,$source='',$destination='',$validate=true,$map=null){
			...
			if( empty($source) || ! $fileC->exist($source)){
				$message=WMessage::get();
				$FILEPATH=$source;
				$message->userE('1305518119BKOP',array('$FILEPATH'=>$FILEPATH));
				return false;				}
	...
	private function _checkUK($new,$reset=false,&$localObj,&$namekeyEID){
		static $countFail=0;
		...
		if( empty($uniquesA) || !is_array($uniquesA)) return true;
			...
			if( $status){
				...
				if( $localObj->getParam('uniqueCorrect')){
					...
					if( $biggestSize>0){
						...
						$biggestValueNew=substr($biggestValueNew, 0, $biggestSize );
					...
					if( 'namekey'==$biggestMap){
						$this->_replaceProperNamekey( $namekeyEID, $biggestValue, $localObj->$biggestMap );
										...
										$this->_checkUK( $new, false, $localObj, $namekeyEID );
	...
	private function _checkUKForChildModel($new,$reset=false,&$localObj,&$namekeyEID){
		...
		if( empty($foundChildsA)) return false;
		...
		foreach( $foundChildsA as $CName=> $oneChild){
			...
			if( is_array($localObj->$CName)){
				...
				foreach( $localObj->$CName as $indexID=> $oneData){
					...
					$this->_checkUK( $new, $reset, $childInstanceI, $namekeyEID );

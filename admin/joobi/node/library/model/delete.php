		...
		foreach( $eids as $eid){
			...
			if( @is_array( $eid )){
				...
				if( $localObj->getAudit()){
					...
					if( $localObj->getAudit()){
						if( !empty($securityAuditA)) $securityAuditA->beforeDelete( @$localObj->$pkey, $localObj->getModelID());
					}
					$pkey=$localObj->getPK();
					$status=$localObj->deleteParent( @$localObj->$pkey );

					if( $localObj->getAudit() && $status){
						if( !empty($securityAuditA)) $securityAuditA->afterDelete( @$localObj->$pkey, $localObj->getModelID());
					}
				}
			}
			if( $status){

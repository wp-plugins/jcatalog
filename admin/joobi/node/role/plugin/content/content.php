...
if(!($urolid))   $urolid=array(1,2);
	...
	if( $uid>0){ 
		...
		if(!empty($route) && isset($route)) $link=WPage::routeURL( $route );

		else $link=WPage::routeURL( 'controller=subscription&task=possible&rolid='. $rolid, '', false, false, true, 'jsubscription' );
		...
		$message=PSUBSCRIPTION_NODE_RESTMESSAGE;

		if(!empty($message) AND isset($message)) $span='<span class="restricted"><a href="'.$link.'">'.$message.'</a></span>';
		...
		$position=PSUBSCRIPTION_NODE_RESTMESSAGEPOS;

		if(!empty($position) AND isset($position) AND $position==true) $article->$text=$article->$text . $image . $span;
		...
		$route=PSUBSCRIPTION_NODE_VREDIRECTLINK;

		if(!empty($route) AND isset($route)) $link=WPage::link( $route );
		...
		$message=PSUBSCRIPTION_NODE_VRESTMESSAGE;
		...
		$position=PSUBSCRIPTION_NODE_VRESTMESSAGEPOS;
		...
		$span='<span class="restricted">'.WText::t('1236926015ATTW').'</span>';

		$article->$text=$image . $span . $article->$text;

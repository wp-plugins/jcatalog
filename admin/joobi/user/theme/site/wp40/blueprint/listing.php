		...
		if ( $this->_magicHeader ) {
			...
			if ( $this->htmlObj->pagination < 11 ) {
				...
				&& $this->htmlObj->pageNavO->pages_total > $this->htmlObj->pageNavO->getDisplayedPages() ) {
					...
					$textMe = '<div class="pull-left pageNb">' . WText::t('1206732366OVLZ').' # </div>';
	...
	private function _createListingHead($searchWord) {
		...
		if ( !empty($this->htmlObj->search) || !empty($this->htmlObj->advSearch) ) {
			...
			if ( !$this->_AdvSearchStatus ) {
				...
				$this->htmlObj->headSeachFooter .= '<div class="panel panel-info filter-advsearch"><div class="panel-heading"><h4 class="panel-title">' . WText::t('1382068744SDQH') . '</h4></div><div class="panel-body">' . $advanceSearchHTML . '</div></div>';

...
//        l=d.mozSlice(f,j);		//before but not working
        l=d.mozSlice(f,j*n);
        }else if(d.webkitSlice){
//        l=d.webkitSlice(f,j)		//before but not working
        l=d.webkitSlice(f,j*n);
        }else if(d.slice){
//        l=d.slice(f,i)	//before but not working
        l=d.slice(f,i*n);
        }else{
        l=d;
        k=true
...
function generateBoundary(){
    ...
    function createPreview(f,g,h,i){
    	var myLanguage = JSON.parse(mytranslation);
        ...
        k.onload=function(e){
            j.css('cursor','pointer').attr('src',e.target.result).click(function(){
                ...
                d.onload=function(){
                    ...
                    $('#ax-box').html('<img width="'+b+'" height="'+c+'" src="'+e.target.result+'" /><a>'+myLanguage.close+'</a><span>'+g.name+'</span>').fadeIn(100).css({
...
var p=$('<div class="ax-progress progress progress-striped" />').appendTo(j);
var q=$('<div class="progress-bar progress-bar-info" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" />').appendTo(p);
var r=$('<div class="ax-progress-info">0%</div>').appendTo(p);
var s=$('<div class="ax-toolbar" />').appendTo(j);
var t=$('<a title="' + myLanguage.start_upload + '" class="btn btn-success ax-btn" />').click(function(){
    if(g.enable)f.sns()
        }).appendTo(s).append('<i class="fa fa-download"></i>');
var u=$('<a title="' + myLanguage.remove_all + '" class="btn btn-danger ax-btn" />').click(function(){
    if(g.enable)f.remove()
        }).appendTo(s).append('<i class="fa fa-times"></i>');
...
var y={
    init:function(r){
        return this.each(function(){
            ...
            d.addClass('ax-uploader').data('author','');
            var f=$.extend({},x,r);
            f.FILES=[];
            f.UPLOADED=0;
            f.SLOTS=0;
            ...
            f.internalFinish=function(a){
                if(g!=null){
                    ...
                    for(var i=0;i<a.length;i++){
                        var c=b+a[i];
                         $.each( f.FILES, function( key, value ) 
                         {
                                var str = {'status':value.status, 'name':value.name, 'size':value.size , 'type':value.file.type};
                                $(g).append("<input name='ax-uploaded-files["+(r.foraxfiles)+"]["+key+"]' type='hidden' value='"+JSON.stringify(str)+"' />")
                        });
    ...
    var h=$('<fieldset />').append('').appendTo(d);
    var j=$(' <a class="btn btn-success ax-btn"  style="position:relative;" title="' + myLanguage.add_files + '" />').appendTo(h);
    j.append('<i class="fa fa-plus"></i> '+myLanguage.add_files );
    var k=$('<input type="file" class="ax-browse" name="ax-files'+r.foraxfiles+'[]" />').attr('multiple',w).appendTo(j);
   // var l=$('<a class="ax-upload-all ax-button" title="Upload all files" />').appendTo(h);
   // l.append('<span class="ax-upload-icon ax-icon"></span> <span>'+myLanguage.start_upload+'</span>');
    var m=$(' <a class="btn btn-danger ax-btn ax-btn" title="' + myLanguage.remove_all + '" />').appendTo(h);
    m.append('<i class="fa fa-times"></i> '+myLanguage.remove_all );
